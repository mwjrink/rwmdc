...
typedef struct StartupStamp { f64 wall, cpu; } StartupStamp;
...
static void startup_phase(const char *name, StartupStamp before, StartupStamp after) {
    printf("Startup %-22s wall %8.3f ms | process CPU %8.3f ms\n",
...
static void report_memory(const TextRenderState *trs, const Arena *arena) {
    ...
    printf("Memory: arena used %.3f MiB | process peak RSS %.3f MiB\n",
           (f64)arena->len / 1048576.0, (f64)usage.ru_maxrss / 1024.0);
    printf("Text Vulkan allocations: assets %.3f MiB | draw buffers %.3f MiB | staging %.3f MiB\n",
           (f64)trs->asset_bytes / 1048576.0, (f64)trs->draw_buffer_bytes / 1048576.0,
...
static void report_samples(const char *label, f64 *values, u32 count) {
    if (!count) { printf("%s: no measured frames\n", label); return; }
    f64 sum = 0;
    u32 misses = 0;
    for (u32 i = 0; i < count; i++) { sum += values[i]; misses += values[i] >= 1.0; }
    qsort(values, count, sizeof(*values), compare_ms);
    printf("%s (%u): mean %.4f ms | p50 %.4f | p95 %.4f | p99 %.4f | max %.4f | >=1ms %u (%.2f%%)\n",
           label, count, sum / count, values[(count - 1) / 2],
           values[(u32)ceil(0.95 * count) - 1], values[(u32)ceil(0.99 * count) - 1],
           values[count - 1], misses, 100.0 * misses / count);
}

static void collect_gpu(const GraphicsContext *gc, VkQueryPool pool, u32 slot,
                        u64 mask, f64 period, f64 *samples, u32 *count) {
    u64 ticks[2];
    check_vkresult(vkGetQueryPoolResults(gc->device, pool, slot * 2, 2,
                   sizeof(ticks), ticks, sizeof(u64), VK_QUERY_RESULT_64_BIT),
                   SCOPE_GFX_COMMAND_BUFFER, "Read completed GPU timestamps");
    samples[(*count)++] = (f64)((ticks[1] - ticks[0]) & mask) * period / 1e6;
}

static u32 layout_benchmark(const AppOptions *opt, const FontState *font,
                           GlyphDrawCmd *draws, u32 capacity, u32 width, u32 height) {
    const char *corpus =
        "# WYSIWYG Markdown - GPU text rendering\n"
        "The quick brown fox jumps over the lazy dog. 0123456789\n"
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz\n"
        "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~\n"
        "**Bold** _italic_ [link](url) `code` - cached outlines\n";
    u32 n = 0;
    f32 advance = font->metrics[font->font->glyph_map['M']].advance_width;
    if (opt->grid) {
        ...
        for (u32 row = 0; row < opt->rows; row++)
            for (u32 col = 0; col < opt->columns; col++) {
                ...
                draws[n++] = (GlyphDrawCmd){
                    .x = (f32)col * cw + (cw - advance * scale) * 0.5f,
                    .y = (f32)row * ch + (ch - font->font->line_height * scale) * 0.5f +
                         font->font->ascent * scale,
                    .sx = scale, .sy = -scale, .glyph_idx = (u32)font->font->glyph_map[cp],
                    .color = 0xffffffff};
            }
        return n;
    }
    f32 x = 12, y = 12 + font->font->ascent;
    const char *p = corpus;
    while (y - font->font->descent <= (f32)height - 12 && n < capacity) {
        if (!*p) p = corpus;
        if (*p == '\n') { x = 12; y += font->font->line_height; p++; continue; }
        u32 gi = (u32)font->font->glyph_map[(u8)*p];
        f32 adv = font->metrics[gi].advance_width;
        if (x + adv > (f32)width - 12) {
            x = 12;
            y += font->font->line_height;
            if (y - font->font->descent > (f32)height - 12) break;
        }
        draws[n++] = (GlyphDrawCmd){.x=x, .y=y, .sx=1, .sy=-1, .glyph_idx=gi, .color=0xffffffff};
        x += adv;
        p++;
    }
    return n;
...
static int run_benchmark(const AppOptions *opt) {
    ...
    if (opt->fullscreen) window_set_fullscreen(target.window);
    ...
    FontState font = font_load(&arena, "assets/fonts/JetBrainsMonoNerdFontMono-Regular.ttf",
                              opt->font_pt > 0 ? opt->font_pt : opt->font_size, opt->font_pt > 0);
    ...
    if (!bits) { fprintf(stderr, "Graphics queue does not support GPU timestamps\n"); return 1; }
    u64 mask = bits == 64 ? UINT64_MAX : ((UINT64_C(1) << bits) - 1);
    VkQueryPoolCreateInfo qi = {.sType=VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO,
        .queryType=VK_QUERY_TYPE_TIMESTAMP, .queryCount=2 * rc.frames_in_flight};
    check_vkresult(vkCreateQueryPool(gc.device, &qi, NULL, &trs.timestamp_pool),
                   SCOPE_GFX_COMMAND_BUFFER, "Create benchmark timestamp pool");
    ...
    while (submitted < opt->warmup + opt->frames) {
        ...
        if (target.window->request_close) break;
        ...
        if (!start_frame(&arena, &rs)) continue;
        ...
        if (size_changed || opt->update_every_frame) {
            draw_count = layout_benchmark(opt, &font, draws, trs.max_draws, last_w, last_h);
            revision++;
        }
        if (opt->animate) pc.scroll_offset.y = 4.0f * (f32)sin((frame_start - animation_start) * 0.001);
        ...
        if (size_changed) { printf("Surface: %ux%u pixels\n", last_w, last_h); fflush(stdout); }
        submitted++;
    }
    ...
    printf("Draw uploads: warmup %.3f KiB | measured %.3f KiB | average %.3f KiB/measured frame\n",
           (f64)warmup_upload / 1024.0, (f64)measured_upload / 1024.0,
...
static void metric_add(Metric *metric, f64 ms) {
    metric->count++; metric->sum += ms;
    if (ms > metric->maximum) metric->maximum = ms;
...
static void metric_report(const char *name, const Metric *metric) {
    printf("%s: %lu samples | mean %.4f ms | max %.4f ms | >=1ms %lu\n", name,
           metric->count, metric->count ? metric->sum / (f64)metric->count : 0,
           metric->maximum, metric->over_budget);
}
...
static void editor_title(Editor *editor, const char *message) {
    char title[2048];
    snprintf(title, sizeof(title), "%s%s — rwmd%s%s",
             editor_dirty(editor) ? "*" : "", editor->path ? editor->path : "Untitled",
...
static bool editor_update_layout(Editor *editor, f32 width, f32 height) {
    bool changed = layout_update(&editor->layout, &editor->document, editor->font,
                                 max(width - 14.0f, 1.0f), height);
    if (editor->layout.failed) { fprintf(stderr, "rwmd: Markdown layout failed\n"); exit(1); }
    if (changed) {
        const LayoutProfile *profile = &editor->layout.profile;
        if (profile->mirror_ns) metric_add_ns(&editor->mirror, profile->mirror_ns);
...
static void editor_changed(Editor *editor) {
    ...
    if (dirty != editor->title_dirty || was_close_armed) editor_title(editor, NULL);
...
static bool editor_replace(Editor *editor, u32 offset, u32 removed, const u8 *text, u32 length) {
    ...
    if (!ok) { editor_error(editor, document_error(&editor->document)); return false; }
    if (editor->document.revision != before) editor_changed(editor);
...
static bool editor_space(const Document *document, u32 offset) {
    if (offset >= document_length(document)) return true;
    u8 byte;
...
static u32 editor_word(const Document *document, u32 cursor, bool forward) {
    ...
    if (forward) {
        while (cursor < length && !editor_space(document, cursor)) cursor = document_next_grapheme(document, cursor);
        while (cursor < length && editor_space(document, cursor)) cursor = document_next_grapheme(document, cursor);
    } else {
        while (cursor) {
            u32 prev = document_prev_grapheme(document, cursor);
            if (!editor_space(document, prev)) break;
            cursor = prev;
        }
        while (cursor) {
            u32 prev = document_prev_grapheme(document, cursor);
...
static void editor_move(Editor *editor, u32 cursor, bool extend) {
    ...
    if (!extend) editor->selection_anchor = cursor;
    layout_reveal(&editor->layout, &editor->document, cursor);
    editor->last_input = monotonic_ms();
}
static void editor_save(Editor *editor) {
    if (!editor->path) { editor_error(editor, "Open with a file path to save this document"); return; }
    if (!document_save(&editor->document, editor->path)) {
        editor_error(editor, document_error(&editor->document)); return;
    }
...
static void editor_copy(Editor *editor, bool cut) {
    ...
    if (!bytes) { editor_error(editor, "Clipboard allocation failed"); return; }
    document_read(&editor->document, begin, end - begin, bytes);
    window_clipboard_set(editor->window, bytes, end - begin);
    free(bytes);
    if (cut) editor_replace(editor, begin, end - begin, NULL, 0);
}
static bool editor_key(Editor *editor, const WindowEvent *event, f32 width, f32 height) {
    ...
    switch (event->key) {
        case WKEY_ESCAPE:
            ...
            return true;
        case WKEY_ENTER:
            return editor_insert(editor, (const u8 *)(editor->document.crlf ? "\r\n" : "\n"), editor->document.crlf ? 2 : 1);
        case WKEY_TAB: return editor_insert(editor, (const u8 *)"\t", 1);
        case WKEY_BACKSPACE: case WKEY_DELETE: {
            u32 begin = min(caret, editor->selection_anchor), end = max(caret, editor->selection_anchor);
            if (begin == end) {
                if (event->key == WKEY_BACKSPACE)
                    begin = ctrl ? editor_word(&editor->document, caret, false) : document_prev_grapheme(&editor->document, caret);
                else end = ctrl ? editor_word(&editor->document, caret, true) : document_next_grapheme(&editor->document, caret);
            }
        ...
        case WKEY_LEFT:
            ...
            next = !shift && editor->selection_anchor != caret ? min(caret, editor->selection_anchor) :
                ctrl ? editor_word(&editor->document, caret, false) : document_prev_grapheme(&editor->document, caret);
            editor->preferred_x = NAN;
            break;
        case WKEY_RIGHT:
            layout_clear_affinity(&editor->layout);
            next = !shift && editor->selection_anchor != caret ? max(caret, editor->selection_anchor) :
                ctrl ? editor_word(&editor->document, caret, true) : document_next_grapheme(&editor->document, caret);
            editor->preferred_x = NAN;
            break;
        case WKEY_HOME: case WKEY_END:
            if (ctrl) layout_clear_affinity(&editor->layout);
            if (!ctrl) layout_reveal(&editor->layout, &editor->document, caret);
            ...
            editor->preferred_x = NAN;
            break;
        case WKEY_UP: case WKEY_DOWN: case WKEY_PAGE_UP: case WKEY_PAGE_DOWN: {
            layout_reveal(&editor->layout, &editor->document, caret);
            editor_update_layout(editor, width, height);
            CaretVisual position = layout_caret(&editor->layout, caret);
            if (!isfinite(editor->preferred_x)) editor->preferred_x = position.x;
...
static void editor_thumb(const Editor *editor, f32 height, f32 *top, f32 *size) {
    ...
    for (u32 i = 0; i < editor->layout.window.line_count; i++) {
        ...
        if (line->y + editor->layout.scroll_y >= height) break;
        if (line->y + line->height + editor->layout.scroll_y > 0) last = line->end;
    }
...
static bool editor_event(Editor *editor, const WindowEvent *event, f32 width, f32 height) {
    ...
    if (event->type == WINDOW_KEY) return editor_key(editor, event, width, height);
    ...
    if (event->type == WINDOW_POINTER_BUTTON && event->button == 1) {
        if (!event->pressed) { editor->selecting = editor->dragging_scrollbar = false; return false; }
        editor_update_layout(editor, width, height);
        if (event->x >= width - 12) {
            f32 top, size; editor_thumb(editor, height, &top, &size);
    ...
    if (event->type == WINDOW_POINTER_MOVE) {
        if (editor->dragging_scrollbar) { editor_drag_thumb(editor, event->y, height); return true; }
        if (editor->selecting) {
            if (event->y < 0) layout_scroll(&editor->layout, &editor->document, event->y);
            else if (event->y > height) layout_scroll(&editor->layout, &editor->document, event->y - height);
            editor_update_layout(editor, width, height);
...
static bool editor_process_events(Editor *editor, f32 width, f32 height) {
    ...
    for (;;) {
        if (editor->paste_pending) {
            ...
            while (index < queue->count && queue->items[index].type != WINDOW_PASTE) index++;
...
static u32 editor_rectangles(Editor *editor, f32 width, f32 height) {
    ...
    if (needed > editor->rect_capacity) {
        ...
        while (cap < needed && cap < UINT32_MAX / 2) cap *= 2;
        if (cap < needed) { fprintf(stderr, "Editor rectangle capacity overflow\n"); exit(1); }
        GpuRect *grown = realloc(editor->rects, (usize)cap * sizeof(*grown));
        if (!grown) { perror("editor rectangles"); exit(1); }
        editor->rects = grown; editor->rect_capacity = cap;
    }
    if (static_count) memcpy(editor->rects, editor->layout.window.rects, (usize)static_count * sizeof(GpuRect));
    u32 count = static_count;
    if (selection_count)
        count += layout_selection_rects(&editor->layout, begin, end, editor->rects + count, editor->rect_capacity - count - 2);
    f32 top, size; editor_thumb(editor, height, &top, &size);
...
static int run_editor(const AppOptions *opt) {
    ...
    } else loaded = document_init(&editor.document, (const u8 *)welcome, (u32)strlen(welcome));
    if (!loaded) {
        fprintf(stderr, "rwmd: %s: %s\n", opt->file ? opt->file : "document", document_error(&editor.document));
        document_destroy(&editor.document); return 1;
    }
    editor.saved_state = document_state_id(&editor.document);
    ...
    if (gc.timestamp_bits)
        check_vkresult(vkCreateQueryPool(gc.device, &qi, NULL, &trs.timestamp_pool), SCOPE_GFX_COMMAND_BUFFER, "Create editor timestamps");
    ...
    while (!opt->frames || presented < opt->frames) {
        ...
        if (target.window->request_close) {
            if (editor_dirty(&editor) && !editor.close_armed) {
                target.window->request_close = false; editor.close_armed = true;
                editor_error(&editor, "Unsaved changes: Ctrl+S saves; close again to discard");
            } else break;
        }
        if (!target.window->width || !target.window->height) { window_wait_events(target.window, 100); continue; }
        if (target.window->width != target.extent.width || target.window->height != target.extent.height) {
            rc.render_target_resized = true; redraw = true;
        }
        ...
        if (blink != previous_blink || previous_focus != target.window->focused) redraw = true;
        previous_blink = blink; previous_focus = target.window->focused;
        ...
        TextFrameStyle style = {.viewport_scale={2.0f/width,2.0f/height}, .text_color={1,1,1,1},
            .cursor_size={max(caret.width,1.0f),caret.height}, .cursor_color=0xffffffff,
            .cursor_visible=caret.visible && blink && target.window->focused && editor.selection_anchor==document_cursor(&editor.document)};
        trs.timestamp_base = slot * 2;
        f64 publication_start = monotonic_ms();
        text_render_frame(&rs, &trs, editor.layout.window.glyphs, editor.layout.window.glyph_count,
                          editor.rects, rect_count, style, pc, editor.layout.window.revision);
        ...
        if (!presented) {
            printf("Editor: %s | %u bytes | GPU: %s\n", opt->file ? opt->file : "Untitled", document_length(&editor.document), gc.properties.deviceName);
            ...
            printf("Surface: %ux%u | input driven; caret=%u | Ctrl+S saves, Ctrl+Z undoes\n", target.extent.width,target.extent.height,document_cursor(&editor.document));
            fflush(stdout);
        }
        presented++;
        redraw = false;
    ...
    for(u32 slot=0; slot<rc.frames_in_flight; slot++) if(pending[slot]) {
        f64 value;u32 count=0;
        collect_gpu(&gc,trs.timestamp_pool,slot,mask,gc.properties.limits.timestampPeriod,&value,&count);
    ...
    if (opt->profile_parser) metric_report("    Callbacks (nested)", &editor.callbacks);
    else printf("    Callback timing disabled; --profile-parser enables per-event clocks.\n");
    ...
    printf("Parser work: %lu full | %lu local | %lu bytes | max %lu bytes/syntax update | %lu events\n",
           editor.full_parses, editor.local_parses, parser->input_bytes, editor.largest_parse_update, parser->event_count);
    printf("Parser arena: %lu allocation requests | %lu reallocations | %.3f KiB copied | %lu chunk growths\n",
           parser->allocations, parser->reallocations, (f64)parser->realloc_copied_bytes / 1024.0, parser->arena_growths);
    printf("Parser scratch: peak %.3f KiB | retained %.3f KiB; reset after each parse.\n",
           (f64)parser->arena_peak_bytes / 1024.0, (f64)parser->arena_capacity_bytes / 1024.0);
    printf("Syntax timing samples are updates, including any local attempt plus full fallback; nested times are not additive.\n");
    metric_report("Editor CPU work", &cpu);
    metric_report("GPU render", &gpu);
    metric_report("Wall frame", &wall);
    printf("Editor frames: %lu | uploaded %.3f KiB | document %u bytes\n", presented, (f64)uploaded/1024.0, document_length(&editor.document));
    report_memory(&trs,&arena);
    if(trs.timestamp_pool) vkDestroyQueryPool(gc.device,trs.timestamp_pool,NULL);
    text_render_cleanup(&gc,&trs);
    cleanup_render_state(&rs); cleanup_render_context(&rc); cleanup_render_target(&gc,&target);
    cleanup_graphics_ctx(&gc); close_window(target.window);
    layout_destroy(&editor.layout); document_destroy(&editor.document); free(editor.rects);
    font_destroy(&font); arena_destroy(&arena);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    for (int i=1; i<argc; i++) {
        ...
        if (!strcmp(arg,"--help")) {
            printf("Usage: %s [file.md] [--file PATH] [--font-pt N | --font-size N]\n"
                   ...
                   "An unsaved document requires closing twice to discard. --frames limits editor rendering for smoke checks.\n"
        ...
        if (!strcmp(arg,"--benchmark")) { opt.benchmark=true; continue; }
        if (!strcmp(arg,"--profile-parser")) { opt.profile_parser=true; continue; }
        if (!strcmp(arg,"--fullscreen")) { opt.fullscreen=true; continue; }
        if (!strcmp(arg,"--animate")) { opt.animate=true; continue; }
        if (!strcmp(arg,"--update-every-frame")) { opt.update_every_frame=true; continue; }
        if (!strcmp(arg,"--")) {
            if (opt.file || i+2!=argc) { fprintf(stderr,"Expected one file after --\n");return 2; }
            opt.file=argv[++i]; break;
        }
        if (arg[0]!='-') {
            if (opt.file) { fprintf(stderr,"Only one document may be opened\n");return 2; }
            opt.file=arg;continue;
        }
        if (++i==argc) { fprintf(stderr,"Missing value for %s\n",arg);return 2; }
        if (!strcmp(arg,"--file")) {
            if (opt.file) { fprintf(stderr,"Only one document may be opened\n");return 2; }
            opt.file=argv[i];continue;
        }
        if (!strcmp(arg,"--scene")) {
            if (!strcmp(argv[i],"grid")) opt.grid=true;
            else if (!strcmp(argv[i],"text")) opt.grid=false;
            else { fprintf(stderr,"Scene must be text or grid\n");return 2; }
            continue;
        }
        char *end;
        if (!strcmp(arg,"--font-pt")) {
            ...
            opt.font_pt=value;size_set=true;continue;
        }
        unsigned long value=strtoul(argv[i],&end,10);
        if (!*argv[i] || *end || value>1000000 || (!value && strcmp(arg,"--warmup"))) {
            fprintf(stderr,"Invalid numeric value for %s: %s\n",arg,argv[i]);return 2;
        }
        if (!strcmp(arg,"--frames")) opt.frames=(u32)value;
        else if (!strcmp(arg,"--warmup")) opt.warmup=(u32)value;
        else if (!strcmp(arg,"--width")) opt.width=(u32)value;
        else if (!strcmp(arg,"--height")) opt.height=(u32)value;
        else if (!strcmp(arg,"--columns")) opt.columns=(u32)value;
        else if (!strcmp(arg,"--rows")) opt.rows=(u32)value;
        else if (!strcmp(arg,"--font-size")) {opt.font_size=(f32)value;opt.font_pt=0;size_set=true;}
        else {fprintf(stderr,"Unknown option: %s\n",arg);return 2;}
    }
    if ((u64)opt.columns*opt.rows>65536 || opt.width<64 || opt.height<64 || opt.width>16384 || opt.height>16384 ||
        (opt.font_pt==0 && (opt.font_size<4 || opt.font_size>512))) {
        fprintf(stderr,"Limits: grid <=65536 cells, extent64..16384, pixel font size4..512\n");return 2;
    }
    if (opt.benchmark) {
        if (opt.file) {fprintf(stderr,"Use document mode with --frames to measure a file\n");return 2;}
        if (opt.profile_parser) {fprintf(stderr,"--profile-parser requires document mode\n");return 2;}
        if (!opt.frames) opt.frames=2000;
